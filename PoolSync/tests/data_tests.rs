...
mod data_test {
    ...
    async fn test_v2_data() {
        ...
        for pool in pools {
            let V2State::getReservesReturn {
                ...
                .unwrap();
            assert_eq!(
                pool.get_v2().unwrap().token0_reserves,
                res0,
                "Address {}, Pool Type {}",
                pool.address(),
                pool.pool_type()
            ...
            assert_eq!(
                pool.get_v2().unwrap().token1_reserves,
                res1,
                "Address {}, Pool Type {}",
                pool.address(),
                pool.pool_type()
    ...
    async fn fetch_v3_pool_data(
        ...
        let (sqrt_price, tick, liquidity, tick_spacing, fee) = match pool_type {
            PoolType::Slipstream => {
                ...
                let V3StateNoFee::slot0Return {
                    ...
                    .await
                    .unwrap();
                let V3StateNoFee::liquidityReturn { _0: liquidity } = contract
                    .liquidity()
                    .block(last_synced_block.into())
                    .call()
                    .await
                    .unwrap();
                let V3StateNoFee::tickSpacingReturn { _0: tick_spacing } = contract
                    .tickSpacing()
                    .block(last_synced_block.into())
                    .call()
                    .await
                    .unwrap();
                let V3StateNoFee::feeReturn { _0: fee } = contract
                    .fee()
                    .block(last_synced_block.into())
                    .call()
                    .await
                ...
                for (tick_key, tick_val) in &pool.ticks {
                    ...
                    assert_eq!(
                        liquidityGross as u128, tick_val.liquidity_gross as u128,
                        "Liquidity Gross at tick {}: Address {}, Pool Type {}",
                        tick_key, pool.address, pool_type
                    );
                    assert_eq!(
                        liquidityNet as i128, tick_val.liquidity_net as i128,
                        "Liquidity Net at tick {}: Address {}, Pool Type {}",
                        tick_key, pool.address, pool_type
                    );
                }
        ...
        assert_eq!(
            pool.tick,
            tick.as_i32(),
            "Tick: Address {}, Pool Type {}",
            pool.address,
            pool_type
        );
        assert_eq!(
            pool.liquidity, liquidity as u128,
            "Liquidity: Address {}, Pool Type {}",
            pool.address, pool_type
        );
        assert_eq!(
            pool.tick_spacing,
            tick_spacing.as_i32(),
            "Tick spacing: Address {}, Pool Type {}",
            pool.address,
            pool_type
        );
        assert_eq!(
